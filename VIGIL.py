"""
VIGIL V7.0 - Vehicle-Installed Guard for Injury Limitation
Complete Professional Web Dashboard with Modern Web Interface
Architecture: React Frontend + FastAPI + gRPC + Kafka Event Bus
...
# Global Kafka producer (initialized on startup)
kafka_producer = None
...
# Try to import RT-DETR (also from ultralytics)
try:
...
# Supported models: model_id → (weight_file, class, description)
SUPPORTED_MODELS = {
    'yolov8n':   {'weights': 'yolov8n.pt',   'description': 'YOLOv8 Nano — fast, lightweight (6.5 GFLOPS)',   'family': 'yolo'},
    'yolov8s':   {'weights': 'yolov8s.pt',   'description': 'YOLOv8 Small — balanced accuracy & speed',       'family': 'yolo'},
    'yolo11n':   {'weights': 'yolo11n.pt',   'description': 'YOLO11 Nano — latest gen, 22% fewer params',     'family': 'yolo'},
    'yolo11s':   {'weights': 'yolo11s.pt',   'description': 'YOLO11 Small — improved feature extraction',     'family': 'yolo'},
    'yolo11m':   {'weights': 'yolo11m.pt',   'description': 'YOLO11 Medium — high accuracy (51.5 mAP)',       'family': 'yolo'},
    'rtdetr-l':  {'weights': 'rtdetr-l.pt',  'description': 'RT-DETR Large — transformer-based detector',     'family': 'rtdetr'},
    'rtdetr-x':  {'weights': 'rtdetr-x.pt',  'description': 'RT-DETR XLarge — max accuracy transformer',      'family': 'rtdetr'},
}

DEFAULT_MODEL = 'yolo11n'  # Default: YOLO11 Nano


class DetectionEngine:
    """Multi-model detection engine with hot-swappable AI models."""
    ...
    def load_model(self, model_id: str = DEFAULT_MODEL) -> bool:
        """Load (or switch to) a detection model by ID. Downloads weights automatically."""
        if model_id not in SUPPORTED_MODELS:
            logger.error(f"Unknown model: {model_id}. Available: {list(SUPPORTED_MODELS.keys())}")
            return False
            ...
            try:
                if family == 'rtdetr':
                    if not RTDETR_AVAILABLE:
                        logger.warning("RT-DETR not available in this ultralytics version, falling back to YOLO")
                        # Fall back to loading via YOLO class (ultralytics handles it)
                ...
                self.model_info = spec
                logger.info(f"Detection engine loaded: {model_id} ({spec['description']})")
                return True
    ...
    def predict(self, frame, conf: float = 0.5, imgsz: int = 320):
        """Run inference. Returns ultralytics Results list."""
        if self.model is None:
            return []
        with self._lock:
            return self.model(frame, conf=conf, verbose=False, imgsz=imgsz, device='cpu', half=False)

    def get_status(self) -> dict:
        return {
            'available': self.available,
            'active_model': self.active_model_id,
            'active_model_info': self.model_info,
            'description': self.model_info.get('description', ''),
            'supported_models': {
                k: {'description': v['description'], 'family': v.get('family', 'unknown')}
...
# FastAPI app
app = FastAPI(
    title="VIGIL V7 Professional",
    description="Vehicle-Installed Guard for Injury Limitation - React + gRPC + Kafka",
    version="7.0.0"
)

# ── Serve React build (static/) if it exists ──
...
# ESP32 Buzzer configuration (optional hardware)
ESP32_BUZZER_CONFIG = {
    'enabled': False,
    'connected': False,
    'ip_address': '',
...
# ESP32 buzzer runtime state
esp32_buzzer_state = {
    'is_playing': False,
    'current_violations': set(),
    'last_violation_time': None,
    'violation_timeout': 2.0,
}

# ── Thread safety locks ──
_state_lock = threading.Lock()        # Protects violations_log, system_stats, event_log
_camera_lock = threading.Lock()       # Protects cameras dict mutations
_recording_lock = threading.Lock()    # Protects camera_recorders / recording_info
_save_lock = threading.Lock()         # Serialises disk writes (zones, violations)
_violations_save_timer = None         # Debounced save timer


def _schedule_save_violations():
    """Debounce violation saves — at most once every 5 seconds."""
    global _violations_save_timer
    if _violations_save_timer is not None:
        _violations_save_timer.cancel()
    _violations_save_timer = threading.Timer(5.0, _do_save_violations)
    _violations_save_timer.daemon = True
    _violations_save_timer.start()


def _do_save_violations():
    """Actually write violations to disk (called from timer thread)."""
    with _save_lock:
        save_violations()


# ── ESP32 Buzzer stub functions ──
# These are safe no-ops unless real ESP32 integration is wired up.
ESP32_BUZZER_SETTINGS = {
    'restricted_sound': 'continuous',
    'warning_sound': 'beep',
}


def trigger_esp32_buzzer(sound_type: str):
    """Send trigger command to ESP32 buzzer hardware.
    Currently a stub — replace with HTTP/serial call to ESP32 when hardware is connected."""
    if not ESP32_BUZZER_CONFIG.get('connected'):
        return
    esp32_buzzer_state['is_playing'] = True
    logger.debug(f"ESP32 buzzer trigger: {sound_type} (stub — no hardware connected)")


def stop_esp32_buzzer():
    """Stop ESP32 buzzer hardware.
    Currently a stub — replace with HTTP/serial call to ESP32 when hardware is connected."""
    esp32_buzzer_state['is_playing'] = False
    esp32_buzzer_state['current_violations'].clear()
    logger.debug("ESP32 buzzer stopped (stub)")
...
async def startup_event():
    ...
    if YOLO_AVAILABLE or RTDETR_AVAILABLE:
        ...
        # Prefer yolo11n if available, fallback to yolov8n if weights already exist
        if not Path(SUPPORTED_MODELS[DEFAULT_MODEL]['weights']).exists() and Path('yolov8n.pt').exists():
            model_to_load = 'yolov8n'
            logger.info("yolo11n weights not found locally — using existing yolov8n.pt (switch in Settings)")
        if detection_engine.load_model(model_to_load):
            logger.info(f"AI detection engine ready: {detection_engine.active_model_id}")
        else:
    ...
    if GRPC_AVAILABLE:
        try:
            grpc_server = start_grpc_server(port=50051, vigil_app=app)
            if grpc_server:
                logger.info("gRPC server started on port 50051")
            else:
                logger.warning("gRPC server not started (stubs not generated)")
        except Exception as e:
    ...
    if KAFKA_AVAILABLE:
        try:
            kafka_producer = VIGILKafkaProducer(
            ...
            if kafka_producer._connected:
                ...
                logger.info("Kafka broker not reachable — running without Kafka")
        except Exception as e:
            logger.info(f"Kafka not available: {e}")
            kafka_producer = None
    else:
        ...
        try:
            ...
            if cap.isOpened():
                ...
                if ret and frame is not None and frame.size > 0:
                    ...
                    logger.debug(f"Camera {device_idx} opened but could not read frames")
                    cap.release()
            else:
                # Only try a few more after first failure if we already found cameras
                if len(available_cameras) > 0 and device_idx > available_cameras[-1]['idx'] + 2:
                    break
        except Exception as e:
    ...
    try:
        ...
        if detection_engine and detection_engine.available and camera_settings.get(camera_id, {}).get('detection_enabled', True):
            ...
            if len(detection_history[camera_id]) == 0 or \
               (now - detection_history[camera_id][-1]['_dt']).total_seconds() > 2:
...
def generate_camera_frame(camera_id: int):
    ...
    while True:
        try:
            # Check if camera still exists and is running
            if camera_id not in cameras or not cameras[camera_id].get('running', False):
                break
...
@app.get("/")
async def read_root():
    """Serve React SPA (if built) or fallback to legacy dashboard"""
    react_index = STATIC_DIR / "index.html"
    if react_index.exists():
        return FileResponse(
            path=str(react_index),
    ...
    # Fallback to legacy inline dashboard
    return HTMLResponse(
...
async def get_system_stats():
    ...
    return JSONResponse(content={
        ...
        'yolo_available': YOLO_AVAILABLE or RTDETR_AVAILABLE,
...
async def export_violations(format: str = 'json'):
    ...
    if format == 'csv':
        ...
        if violations:
            ...
            all_keys = list(dict.fromkeys(k for v in violations for k in v.keys()))
...
async def get_ai_models():
    """List available AI models and which one is active."""
    engine_status = detection_engine.get_status() if detection_engine else {
        'available': False,
        'active_model': '',
    ...
    return JSONResponse(content=engine_status)


@app.post("/api/ai/models/switch")
async def switch_ai_model(request: dict):
    """Switch to a different AI model at runtime.
    Body: { "model_id": "yolo11n" }
    """
    model_id = request.get('model_id', '')
    if not model_id:
        raise HTTPException(status_code=400, detail="model_id required")
    if model_id not in SUPPORTED_MODELS:
        raise HTTPException(status_code=400, detail=f"Unknown model. Choose from: {list(SUPPORTED_MODELS.keys())}")
    if not detection_engine:
        raise HTTPException(status_code=503, detail="Detection engine not initialized")

    logger.info(f"Switching AI model to: {model_id}")
    loop = asyncio.get_event_loop()
    success = await loop.run_in_executor(None, detection_engine.load_model, model_id)
    ...
    event_log.append({
        ...
        'description': f'AI model switched to {model_id} ({SUPPORTED_MODELS[model_id]["description"]})',
        'confidence': 1.0
    })

    return JSONResponse(content={
        'status': 'success',
        'model': detection_engine.get_status()
    })
...
def get_complete_dashboard_html():
    """Generate Bento Grid professional HTML dashboard with ALL features on ONE page"""
    return """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>VIGIL V6.0 - Control Center</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
        /* ════════════════════════════════════════════
           VIGIL V6.0 — BENTO GRID DARK THEME
           Only visual reskin — all functionality intact
           ════════════════════════════════════════════ */
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @keyframes tamper-alert {
            0%,100% { border-color: var(--red); box-shadow: 0 0 10px rgba(239,68,68,0.5); }
            50% { border-color: var(--accent); box-shadow: 0 0 20px rgba(251,191,36,0.7); }
        }
        @keyframes slideIn { from { transform: translateX(40px); opacity:0; } to { transform: translateX(0); opacity:1; } }
        @keyframes fadeOut { from { opacity:1; } to { opacity:0; } }

        .recording-active { animation: pulse 1s infinite !important; }
        .tamper-warning { animation: tamper-alert 0.5s infinite !important; border: 4px solid var(--red) !important; }
        ...
        .title-section h1 { font-size: 18px; font-weight: 700; letter-spacing: -0.5px; color: var(--text); }
        .subtitle { font-size: 11px; color: var(--text-dim); }

        .display-mode-toggle { display: flex; gap: 4px; background: var(--bg); padding: 3px; border-radius: 8px; }
        .display-mode-btn {
            padding: 5px 10px; border: none; background: transparent; color: var(--text-dim);
            font-size: 14px; cursor: pointer; border-radius: 6px; transition: all 0.2s;
        }
        .display-mode-btn:hover { background: var(--surface2); }
        .display-mode-btn.active { background: var(--surface2); color: var(--accent); }

        .touch-toggle {
            display: flex; align-items: center; gap: 6px;
            padding: 4px 10px; border-radius: 8px; background: var(--bg);
        }
        .touch-icon { font-size: 14px; }
        .touch-toggle-label { font-size: 11px; color: var(--text-dim); font-weight: 500; }
        .touch-switch {
        ...
        .touch-switch.active { background: var(--green); }
        .touch-switch.active::after { transform: translateX(16px); }

        .status-badge {
            padding: 5px 14px; border-radius: 20px; font-size: 11px; font-weight: 700;
            letter-spacing: 0.5px; background: var(--green); color: #fff;
        }
        .shutdown-btn {
            width: 34px; height: 34px; border-radius: 8px; border: 1px solid var(--border);
            background: var(--surface2); color: var(--text-dim); font-size: 16px;
            cursor: pointer; transition: all 0.2s; display: grid; place-items: center;
        }
        .shutdown-btn:hover { border-color: var(--red); color: var(--red); background: rgba(239,68,68,0.1); }
        ...
        .tab-btn, .tab-button {
            padding: 7px 20px; border: none; background: transparent; color: var(--text-dim);
            font-size: 12px; font-weight: 600; cursor: pointer; border-radius: 8px;
            transition: all 0.2s; font-family: inherit;
        }
        .tab-btn:hover { color: var(--text); background: var(--surface2); }
        .tab-btn.active, .tab-button.active { color: var(--accent); background: var(--surface2); }
        ...
        .left-panel::-webkit-scrollbar-thumb, .right-panel::-webkit-scrollbar-thumb { background: var(--border); border-radius: 2px; }

        .center-section { overflow-y: auto; scrollbar-width: thin; scrollbar-color: var(--border) transparent; }
        .center-section::-webkit-scrollbar { width: 4px; }
        .center-section::-webkit-scrollbar-thumb { background: var(--border); border-radius: 2px; }
        ...
        .panel-header-compact { padding: 8px 12px; }

        /* ═══ POWER SWITCH ═══ */
        .power-switch { position: relative; display: inline-block; width: 44px; height: 24px; }
        ...
        .power-switch input:checked + .power-slider { background: var(--green); }
        .power-switch input:checked + .power-slider::before { transform: translateX(20px); }

        /* ═══ STATS ═══ */
        .stats-compact { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; padding: 10px; }
        .stat-card {
        ...
        .camera-title { font-size: 13px; font-weight: 600; color: var(--text); }
        .camera-status { display: flex; align-items: center; gap: 6px; font-size: 11px; color: var(--green); font-weight: 600; }
        .status-dot {
        ...
        .camera-controls {
            padding: 10px !important; display: flex !important; gap: 8px !important;
            flex-wrap: wrap !important;
            background: var(--surface2) !important;
            border-top: 1px solid var(--border) !important;
        }
        .control-btn, .camera-control-btn {
            flex: 1 !important; min-width: 90px !important;
            padding: 8px 12px !important;
            color: white !important; border: none !important;
            border-radius: var(--radius-sm) !important;
            font-size: 12px !important; font-weight: 600 !important;
            cursor: pointer !important; transition: filter 0.15s !important;
            font-family: inherit !important;
        }
        .control-btn:hover, .camera-control-btn:hover { filter: brightness(1.15) !important; }
        .camera-stats {
        ...
        .camera-stat-item:last-child { border-right: none; }
        .camera-stat-label { font-size: 10px; color: var(--text-muted); }
        .camera-stat-value { font-size: 16px; font-weight: 700; color: var(--text); margin-top: 2px; }
        ...
        .log-time { font-size: 10px; color: var(--text-muted); font-weight: 500; }
        .log-message { font-size: 11px; color: var(--text-dim); line-height: 1.4; margin-top: 2px; }
        ...
        .setting-label { font-size: 12px; color: var(--text-dim); display: flex; align-items: center; gap: 8px; }
        ...
        .setting-slider::-webkit-slider-thumb, input[type="range"]::-webkit-slider-thumb {
        ...
        .zone-tool-btn:hover { border-color: var(--accent); color: var(--text); }
        .zone-tool-btn.active { background: var(--accent); color: #000; border-color: var(--accent); }
        .zone-canvas-container { position: relative; margin: 12px; background: #000; border-radius: var(--radius-sm); overflow: hidden; }
        .zone-list { padding: 12px; }
        .zone-item {
            display: flex; align-items: center; justify-content: space-between;
        ...
        .zone-item-type { font-size: 11px; color: var(--text-muted); margin-top: 2px; }
        .zone-item-actions { display: flex; gap: 6px; }
        .zone-action-btn {
        ...
        .zone-action-btn:hover { border-color: var(--red); color: var(--red); }
        ...
        .camera-detail-header h3 { color: var(--accent) !important; }

        /* ═══ EVENT HISTORY ═══ */
        #eventHistory {
            padding: 8px;
        }
        #eventHistory .log-entry {
            background: var(--surface);
            border: 1px solid var(--border);
        ...
        body.touch-mode .camera-controls { gap: 12px !important; padding: 15px !important; }
        body.touch-mode .camera-card { margin-bottom: 15px; }
        body.touch-mode .camera-grid { grid-template-columns: 1fr !important; gap: 15px !important; }
        body.touch-mode .camera-feed { height: 300px !important; }
        body.touch-mode .tab-nav { overflow-x: auto; }
        body.touch-mode .tab-btn { white-space: nowrap; }

        /* ═══ COMPACT MODE ═══ */
        body.compact-mode .main-container { grid-template-columns: 1fr; }
        body.compact-mode .left-panel, body.compact-mode .right-panel { display: none; }
        body.compact-mode .camera-grid { grid-template-columns: repeat(2, 1fr); }
        body.compact-mode .camera-feed { height: 200px; }
        body.compact-mode .header { padding: 6px 12px; }
        body.compact-mode .tab-nav { padding: 4px 12px; }

        /* ═══ ULTRA COMPACT MODE ═══ */
        body.ultra-compact-mode .main-container { grid-template-columns: 1fr; }
        body.ultra-compact-mode .left-panel, body.ultra-compact-mode .right-panel { display: none; }
        body.ultra-compact-mode .camera-grid { grid-template-columns: repeat(2, 1fr); gap: 4px; }
        body.ultra-compact-mode .camera-feed { height: 160px; }
        body.ultra-compact-mode .camera-header { padding: 4px 8px; }
        body.ultra-compact-mode .camera-title { font-size: 10px; }
        body.ultra-compact-mode .camera-stats { display: none; }
        body.ultra-compact-mode .camera-controls { padding: 4px !important; gap: 4px !important; }
        body.ultra-compact-mode .control-btn { padding: 4px 6px !important; font-size: 10px !important; min-width: 60px !important; min-height: auto !important; }
        body.ultra-compact-mode .header { padding: 4px 10px; }
        body.ultra-compact-mode .header h1 { font-size: 14px; }
        body.ultra-compact-mode .tab-nav { padding: 2px 10px; }
        body.ultra-compact-mode .tab-btn { padding: 4px 10px; font-size: 10px; }
        ...
        [style*="color: #888"] { color: var(--text-muted) !important; }
        [style*="color: #1a1a1a"], [style*="color: #1f2937"] { color: var(--text) !important; }
        [style*="color: #4b5563"] { color: var(--text-dim) !important; }
        [style*="color: #6b7280"] { color: var(--text-dim) !important; }
        [style*="background: #ffffff"], [style*="background: white"] { background: var(--surface) !important; }
        [style*="background: #0a0a0a"] { background: var(--bg) !important; }
        [style*="background: #374151"] { background: var(--surface2) !important; }
        [style*="background: #1a1a2e"] { background: var(--bg) !important; }
        [style*="border-bottom: 1px solid #333"] { border-bottom: 1px solid var(--border) !important; }
        [style*="border-top: 2px solid #e5e7eb"] { border-top: 1px solid var(--border) !important; }
        [style*="border-top: 1px solid #e5e7eb"] { border-top: 1px solid var(--border) !important; }
        [style*="background: #e5e7eb"] { background: var(--bg) !important; }
        [style*="border: 1px solid #d1d5db"] { border: 1px solid var(--border) !important; }
        a { color: var(--accent); }

        /* Fix dialog/modal styles for dark theme */
        [style*="background: white; padding: 25px"] { 
            background: var(--surface) !important; color: var(--text) !important;
            border: 1px solid var(--border) !important;
        }
        [style*="background: rgba(0,0,0,0.5)"] { background: rgba(0,0,0,0.6) !important; backdrop-filter: blur(4px); }

        /* ═══ LIGHT THEME OVERRIDES ═══ */
        [data-theme="light"] [style*="color: #888"] { color: var(--text-muted) !important; }
        [data-theme="light"] [style*="color: #1a1a1a"], [data-theme="light"] [style*="color: #1f2937"] { color: var(--text) !important; }
        [data-theme="light"] [style*="color: #4b5563"] { color: var(--text-dim) !important; }
        [data-theme="light"] [style*="color: #6b7280"] { color: var(--text-dim) !important; }
        [data-theme="light"] [style*="background: #ffffff"], [data-theme="light"] [style*="background: white"] { background: var(--surface) !important; }
        [data-theme="light"] [style*="background: #0a0a0a"] { background: var(--bg) !important; }
        [data-theme="light"] [style*="background: #374151"] { background: var(--surface2) !important; }
        [data-theme="light"] [style*="background: #1a1a2e"] { background: var(--bg) !important; }
        [data-theme="light"] [style*="border-bottom: 1px solid #333"] { border-bottom: 1px solid var(--border) !important; }
        [data-theme="light"] [style*="border-top: 2px solid #e5e7eb"] { border-top: 1px solid var(--border) !important; }
        [data-theme="light"] [style*="background: #e5e7eb"] { background: var(--border) !important; }
        [data-theme="light"] [style*="background: white; padding: 25px"] {
            background: var(--surface) !important; color: var(--text) !important;
            border: 1px solid var(--border) !important;
        }
        [data-theme="light"] .logo-icon { box-shadow: 0 0 15px rgba(245,158,11,0.2); }
        [data-theme="light"] .camera-feed { background: var(--cam-feed-bg); }
        [data-theme="light"] .status-dot { box-shadow: 0 0 4px var(--green); }
        [data-theme="light"] a { color: var(--accent); }
        [data-theme="light"] .setting-slider, [data-theme="light"] input[type="range"] { background: var(--slider-track); }
        ...
        .theme-btn:hover { border-color: var(--accent); color: var(--accent); background: rgba(245,158,11,0.1); }
    </style>
</head>
<body>
    <div class="header">
        ...
        <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <button class="theme-btn" id="themeToggle" onclick="toggleTheme()" title="Toggle Light/Dark">🌙</button>
    ...
    <script>
        ...
        function toggleTheme() {
            ...
            localStorage.setItem('vigilTheme', next);
            document.getElementById('themeToggle').textContent = next === 'dark' ? '🌙' : '☀️';
        }

        function initTheme() {
            const saved = localStorage.getItem('vigilTheme') || 'dark';
...
async def shutdown_event():
    """Clean up cameras, recordings, gRPC, and Kafka on shutdown"""
    logger.info("Shutting down VIGIL — releasing resources...")

    # Stop all camera capture threads and release hardware
    for cam_id, cam_data in list(cameras.items()):
        try:
            cam_data['running'] = False
            # Stop active recordings to flush video files
            if cam_id in camera_recorders and camera_recorders[cam_id] is not None:
    ...
    if kafka_producer:
        try:
            ...
            pass
    logger.info("VIGIL V7.0 shutdown complete")


if __name__ == "__main__":
    import uvicorn
    logger.info("Starting VIGIL V7.0 Professional — React + gRPC + Kafka")
